...
async function sha256(text){ const buf=await crypto.subtle.digest('SHA-256',new TextEncoder().encode('dearest-pin:'+text)); return [...new Uint8Array(buf)].map(b=>b.toString(16).padStart(2,'0')).join(''); }
...
document.addEventListener('click',async e=>{
  ...
  if(action==='clear'){ if(confirm('Clear Dearest data on this device?')){ localStorage.removeItem(STORE); location.reload(); } }
});
...
function memoryRow(m){return `<button class="memory" data-action="open-memory" data-id="${m.id}"><div class="avatar">${m.type==='voice'?'◉':'✉'}</div><div><strong>${esc(m.title)}</strong><small>${esc(m.person||'Dearest')} · ${prettyDate(m.date)} · Tap to open</small></div></button>`}
