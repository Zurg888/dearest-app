const STORE = 'kinkeep-v1';
const LEGACY_STORE = 'dearly-v1';
if(!localStorage.getItem(STORE) && localStorage.getItem(LEGACY_STORE)) localStorage.setItem(STORE, localStorage.getItem(LEGACY_STORE));
const state = load();
...
async function sha256(text){ const buf=await crypto.subtle.digest('SHA-256',new TextEncoder().encode('kinkeep-pin:'+text)); return [...new Uint8Array(buf)].map(b=>b.toString(16).padStart(2,'0')).join(''); }
...
document.addEventListener('click',async e=>{
  ...
  if(action==='clear'){ if(confirm('Clear KinKeep data on this device?')){ localStorage.removeItem(STORE); location.reload(); } }
});
...
function renderWelcome(){ screen.innerHTML=`<section class="hero"><img class="hero-logo" src="./assets/kinkeep-logo.jpeg" alt="KinKeep logo"><p class="eyebrow">For every someday</p><h1>Keep what matters, beautifully.</h1><p>Save letters, voice notes, photos, and memories for the people you love — private by default, warm by design.</p><button class="primary" data-action="start">Begin your vault</button></section>`; }
function renderHome(){ const recent=state.memories.slice().reverse().slice(0,3); screen.innerHTML=`<section class="hero" style="min-height:300px"><img class="hero-logo" style="width:78px;height:78px" src="./assets/kinkeep-logo.jpeg" alt=""><p class="eyebrow">Good ${greeting()}</p><h2 class="serif">Your words, kept close.</h2><p>One letter, one voice note, one photo. Build a private family keepsake over time.</p><button class="primary" data-tab="write">Write a letter</button></section><section class="grid two" style="margin-top:14px">${quick('✎','New letter','Words for someday','write')}${quick('◉','Voice note','Save your voice','record')}${quick('◇','Memory','Photo or story','write')}${quick('⌁','Delivery','Private or later','settings')}</section><div class="section-title"><h3>Loved ones</h3><button class="small-btn" data-action="add-person">Add</button></div><div class="list">${state.people.slice(0,3).map(personRow).join('')}</div><div class="section-title"><h3>Recent keepsakes</h3><span class="pill">${state.memories.length} saved</span></div><div class="list">${recent.length?recent.map(memoryRow).join(''):'<div class="card empty">No keepsakes yet. Start with one letter.</div>'}</div>`; }
...
function personRow(p){return `<article class="person"><div class="avatar">${esc((p.name||'K')[0])}</div><div><strong>${esc(p.name)}</strong><small>${esc(p.relation)} · ${esc(p.note||'Private keepsakes')}</small></div></article>`}
function memoryRow(m){return `<button class="memory" data-action="open-memory" data-id="${m.id}"><div class="avatar">${m.type==='voice'?'◉':'✉'}</div><div><strong>${esc(m.title)}</strong><small>${esc(m.person||'KinKeep')} · ${prettyDate(m.date)} · Tap to open</small></div></button>`}
